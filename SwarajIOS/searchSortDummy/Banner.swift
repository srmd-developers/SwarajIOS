...
open class Banner: UIView {
    ...
    private func resetTintColor() {
        ...
        imageView.image = image//shouldTintImage ? image?.withRenderingMode(.alwaysTemplate) : image
    ...
    open func show(_ view: UIView? = nil, duration: TimeInterval? = nil) {
        ...
        timer.invalidate()
        if (!timer.isValid) {
            let aSelector : Selector = #selector(Banner.playSound)
            timer = Timer.scheduledTimer(timeInterval: 13.0, target: self, selector: aSelector, userInfo: nil, repeats: true)
            
        }
        var soundURL: NSURL?
        let filePath = Bundle.main.path(forResource: "bell", ofType: "mp3")
